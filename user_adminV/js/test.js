window.onload = function() {
  ...
  formulario.onsubmit= e=>{
    ...
    //llama al metodo crear pregunta y pasa el formulario    
    crearPregunta(formulario);
    //oculta el formulario para insertar pregunta 
    document.querySelector(".contenedor_formulario__test").style.display = "none";
    
   setTimeout(function(){ obtenerPreguntaz(valorSeleccionado);},500);
   

  }
  //capturamos el boton guardar y sequir editando con su clase, agregamos evento clic
  document.querySelector('.saveAdd').addEventListener('click', function(e){
    console.log('presionaste guardar y editar');
    
    //evitamos que se vaya al envio
    e.preventDefault( );
    //hacemos visible el formulario
    document.querySelector(".contenedor_formulario__test").style.display = "flex";
    // llama al metodo crear pregunta y pasa el formulario
    crearPregunta(formulario);
  });
     ...
     for(let datos of preguntasRegistradas) {
      ...
      for(let i=0; i<btnQuitar.length; i++){
        ...
        btnAct[i].addEventListener('click', function(){
          document.querySelector(".contenedor_formulario__test").style.display = "block";
          //llamamos al metodo eliminaPreguntra y le pasamos el item seleccionado a traves de su atributo
...
function actualizarPreguntra(item){
  //se hace visible el m¿formulario
  document.querySelector(".contenedor_formulario__test").style.display = "flex";
...
document.getElementById("nuevaPregunta").addEventListener("click", function() {
  document.querySelector(".contenedor_formulario__test").style.display = "block";
});
